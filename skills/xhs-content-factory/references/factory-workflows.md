...
- Prefer stable filenames: `video.mp4`, `video_cover.jpg`, `cover.jpg` or `cover.png`, `audio_16k.wav`, `subtitle_zh-CN.srt`, `subtitle_en-US.srt`, `transcript.txt`, and `transcript_segments.json`.
...
- For video benchmarks, the `封面` Base field must use a frame extracted from `video.mp4`, not a Xiaohongshu/Douyin platform placeholder or share-card image. Prefer this skill's `scripts/xhs_extract_video_cover.py <asset-dir> [timestamp]` helper to create `video_cover.jpg`, then upload `video_cover.jpg` to `封面`. Keep platform `cover.jpg` locally as raw source material only unless no video is available.
