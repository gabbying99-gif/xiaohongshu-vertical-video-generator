...
Use `D:\Codex\xhs-content-assets` as the default local asset root on this user's Windows machine. For Xiaohongshu web links, first fetch the shared page with a browser-like user agent and check whether a real subtitle/transcript source is exposed (`.srt`, `subtitles`, or equivalent). Report transcript availability before relying on the transcript field. Then parse available metadata from HTML/JSON-LD/state payloads: title, author, description, tags, interaction counts, duration, video URL, platform cover URL, and subtitle URLs. Download available video/subtitle/cover files into `D:\Codex\xhs-content-assets\downloads\<platform>_<work_id>\`. For video benchmarks, generate `video_cover.jpg` from `video.mp4` and upload that to the Base `封面` field; do not upload Xiaohongshu/Douyin placeholder or share-card cover images as the Base cover. Upload downloaded videos to matching Base attachment fields when the table has them. If no platform subtitle is exposed but a video is available, extract audio with ffmpeg and run local ASR to create `audio_16k.wav`, `transcript.txt`, and `transcript_segments.json`; upload `transcript.txt` and write the ASR text into `逐字稿`, clearly marking it as ASR output. If D: is unavailable, fall back to the workspace `downloads/` folder and report that fallback. If parsing or downloading fails because of login, anti-bot, expired links, or network limits, archive the benchmark with known fields and ask the user for the video file, screenshots, transcript, or a fresh link.
